...
const rejectMahasiswaController = async (req, res, next) => {
  try {
    ...
    const result = await rejectMahasiswa(id_user, req.body.catatan);
    if (result.error) {
      const status = result.message.includes("tidak ditemukan") ? 404 : 400;
      return res.status(status).json({ success: false, message: result.message, data: result.data });
    }
...
const getPendingDosenController = async (req, res, next) => {
  try {
    const filters = {
      status_verifikasi: req.query.status_verifikasi !== undefined ? parseInt(req.query.status_verifikasi) : undefined,
      email_verified: req.query.email_verified !== undefined ? req.query.email_verified === "true" : undefined,
    ...
    const result = await listPendingDosen(filters);
    return res.status(200).json({ success: true, message: result.message, data: result.data });
  } catch (err) { next(err); }
};

const getDetailDosenController = async (req, res, next) => {
  try {
    const id_user = parseInt(req.params.id);
    if (isNaN(id_user) || id_user <= 0) {
    ...
    const result = await detailDosen(id_user);
    if (result.error) return res.status(404).json({ success: false, message: result.message, data: null });
    return res.status(200).json({ success: true, message: result.message, data: result.data });
  } catch (err) { next(err); }
};

const approveDosenController = async (req, res, next) => {
  try {
    const id_user = parseInt(req.params.id);
    if (isNaN(id_user) || id_user <= 0) {
      return res.status(400).json({ success: false, message: "ID dosen tidak valid", data: null });
    ...
    const result = await approveDosen(id_user);
    if (result.error) {
      const status = result.message.includes("tidak ditemukan") ? 404 : 400;
      return res.status(status).json({ success: false, message: result.message, data: result.data });
    }
...
const rejectDosenController = async (req, res, next) => {
  try {
    const id_user = parseInt(req.params.id);
    if (isNaN(id_user) || id_user <= 0) {
      return res.status(400).json({ success: false, message: "ID dosen tidak valid", data: null });
    ...
    const result = await rejectDosen(id_user, req.body.catatan);
    if (result.error) {
      const status = result.message.includes("tidak ditemukan") ? 404 : 400;
      return res.status(status).json({ success: false, message: result.message, data: result.data });
    }
