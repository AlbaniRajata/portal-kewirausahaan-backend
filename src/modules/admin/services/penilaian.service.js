...
const getListProposalRekapTahap1 = async (id_program) => {
  const data = await getListProposalRekapTahap1Db(id_program);
  return { error: false, message: "List proposal rekap tahap 1 berhasil diambil", data };
};

const getListProposalRekapTahap2 = async (id_program) => {
  const data = await getListProposalRekapTahap2Db(id_program);
  return { error: false, message: "List proposal rekap tahap 2 berhasil diambil", data };
};

const getRekapDeskEvaluasi = async (id_program, id_proposal) => {
  const rows = await getRekapReviewerTahap1Db(id_program, id_proposal);
  if (!rows.length) return { error: true, message: "Belum ada penilaian reviewer yang disubmit", data: { id_proposal } };
  ...
  return { error: false, message: "Rekap desk evaluasi tahap 1 berhasil diambil", data: { proposal, reviewer: Object.values(reviewerMap) } };
};
...
const finalisasiDeskBatch = async (id_program, payload) => {
  ...
  if (!Array.isArray(lolos) || !Array.isArray(tidak_lolos)) {
    return { error: true, message: "Payload batch tidak valid", data: null };
  }

  const hasil = [];

  for (const id_proposal of lolos) {
    const [totalDistribusi, totalSubmit] = await Promise.all([
...
const getRekapWawancaraTahap2 = async (id_program, id_proposal) => {
  const [reviewerRows, juriRows] = await Promise.all([
    getRekapReviewerTahap2Db(id_program, id_proposal),
    getRekapJuriTahap2Db(id_program, id_proposal),
  ]);
...
const finalisasiWawancaraBatch = async (id_program, payload) => {
  ...
  if (!Array.isArray(lolos) || !Array.isArray(tidak_lolos)) {
    return { error: true, message: "Payload tidak valid", data: null };
  }
  ...
  for (const id_proposal of lolos) {
    const [totalDistribusi, totalSubmit] = await Promise.all([
      countDistribusiPanelTahap2Db(id_program, id_proposal),
      countSubmittedPanelTahap2Db(id_program, id_proposal),
    ]);
    if (totalDistribusi === 0 || totalSubmit !== totalDistribusi) continue;
    ...
    try {
      ...
      if (proposalTim) await insertPesertaProgramByTimDb(client, proposalTim.id_tim, id_program);
      await client.query("COMMIT");
      hasil.push({ id_proposal, status: "LOLOS WAWANCARA", updated });
    } catch (err) {
      await client.query("ROLLBACK");
      hasil.push({ id_proposal, status: "ERROR", error: err.message });
    } finally {
      client.release();
    }
  }
  ...
  for (const id_proposal of tidak_lolos) {
    ...
    try {
      ...
      await client.query("COMMIT");
      hasil.push({ id_proposal, status: "TIDAK LOLOS WAWANCARA", updated });
    } catch (err) {
      await client.query("ROLLBACK");
      hasil.push({ id_proposal, status: "ERROR", error: err.message });
    } finally {
  ...
  return { error: false, message: "Finalisasi wawancara tahap 2 berhasil", data: hasil };
};
