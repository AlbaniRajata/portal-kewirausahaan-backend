...
const getRekapWawancaraTahap2 = async (id_program, id_proposal) => {
  ...
  const proposal = {
    id_proposal:
      reviewerRows[0]?.id_proposal || juriRows[0]?.id_proposal,
    judul: reviewerRows[0]?.judul || juriRows[0]?.judul,
  };

  const reviewer = groupPenilaian(
    reviewerRows,
    "id_reviewer",
    "nama_reviewer"
  );

  const juri = groupPenilaian(
    juriRows,
    "id_juri",
    "nama_juri"
  );

  const totalReviewer = reviewer.reduce(
    (sum, r) => sum + r.total_nilai,
    0
  );

  const totalJuri = juri.reduce(
    (sum, j) => sum + j.total_nilai,
    0
  );
...
const finalisasiWawancaraBatch = async (id_program, payload) => {
  ...
  for (const id_proposal of lolos) {
    const totalDistribusi =
      await countDistribusiPanelTahap2Db(id_program, id_proposal);

    const totalSubmit =
      await countSubmittedPanelTahap2Db(id_program, id_proposal);
    if (totalDistribusi === 0) continue;
    if (totalSubmit !== totalDistribusi) continue;

    const updated =
      await updateStatusProposalTahap2Db(id_program, id_proposal, 8);
  ...
  for (const id_proposal of tidak_lolos) {
    ...
    const updated =
      await updateStatusProposalTahap2Db(id_program, id_proposal, 7);
