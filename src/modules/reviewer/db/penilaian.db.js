...
const getOrCreatePenilaianDb = async (id_distribusi, id_tahap) => {
  const insert = `
    INSERT INTO t_penilaian (id_distribusi, id_tahap)
    VALUES ($1, $2)
  ...
  const q = `
    SELECT *
    FROM t_penilaian
    WHERE id_distribusi = $1 AND id_tahap = $2
  `;
  const { rows } = await pool.query(q, [id_distribusi, id_tahap]);
  return rows[0];
};
