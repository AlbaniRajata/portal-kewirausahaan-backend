...
const getPenugasanController = async (req, res) => {
  ...
  const result = await getPenugasan(id_user);
...
const getDetailPenugasanController = async (req, res) => {
  ...
  if (!id_distribusi) {
    return res.status(400).json({
      ...
      data: { field: "id_distribusi", reason: "id_distribusi wajib diisi" },
    });
  }
...
const acceptPenugasanController = async (req, res) => {
  ...
  res.json({
    message: "Penugasan juri diterima",
    data: result.data,
  });
};
...
const rejectPenugasanController = async (req, res) => {
  ...
  if (!catatan) {
    return res.status(400).json({
      message: "Validasi gagal",
      data: { field: "catatan", reason: "Catatan wajib diisi" },
    });
  }
  ...
  res.json({
    message: "Penugasan juri ditolak",
    data: result.data,
  });
};
